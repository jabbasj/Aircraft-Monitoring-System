...
void* AircraftMonitoringSystem::PollSensors(void) {
	...
	bool FuelTriggered = false, PressureTriggered = false, TemperatureTriggered = false, Smoke1Triggered = false, Smoke2Triggered = false;
		...
		if (rcvid_c == 0)
		{
			...
			if (m_SmokeDetector1.Poll1() == false) {
				...
				if (!Smoke1Triggered) {
					...
					StartInterrupt(m_SmokeDetector1.GetLatestError(),&m_SmokeDetector1, true, true);
				...
				if (Smoke1Triggered) {
					...
					StartInterrupt(std::string("SMOKE FROM DETECTOR 1 CLEARED"),&m_SmokeDetector1, false, true);
			...
			if (m_SmokeDetector2.Poll2() == false) {
				...
				if (!Smoke2Triggered) {
					...
					StartInterrupt(m_SmokeDetector2.GetLatestError(),&m_SmokeDetector2, true, true);
				...
				if (Smoke2Triggered) {
					...
					StartInterrupt(std::string("SMOKE FROM DETECTOR 2 CLEARED"),&m_SmokeDetector2, false, true);
...
void AircraftMonitoringSystem::StartInterrupt(std::string msg, SystemInput* sensor, bool disp, bool save) {

	InterruptData * inter_info = new InterruptData(msg, sensor, this, disp, save);
	pthread_t temp;
...
void* AircraftMonitoringSystem::KeyboardHandler(void) {
	...
	while (1) {
		...
		if (option == 'p') {
			std::vector<std::string> rts = ResponseTime::response_times;
...
std::vector<std::string> ResponseTime::response_times;

int ResponseTime::Compute(std::string type) {
	response_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count();

	std::string msg = type + ":: " + std::to_string(response_time);
	ResponseTime::response_times.push_back(msg);
